...
var mathLibrary = []RegistryFunction{
	...
	{"log", func(l *State) int {
		...
		} else if base := l.CheckNumber(2); base == 10.0 {
	...
	{"random", func(l *State) int {
		...
		case 1: // upper limit only
			u := l.CheckNumber(1)
			l.ArgumentCheck(1.0 <= u, 1, "interval is empty")
			l.PushNumber(math.Floor(r*u) + 1.0) // [1, u]
		case 2: // lower and upper limits
			lo, u := l.CheckNumber(1), l.CheckNumber(2)
			l.ArgumentCheck(lo <= u, 2, "interval is empty")
