...
| Workload | Native Go | Luart (JIT) | Luart (no JIT) | go-lua | Luart (JIT) vs Go | Luart (no JIT) vs Go | go-lua vs Go |
|---|---|---|---|---|---|---|---|
| fib(25), recursive calls | 0.25 ms | 2.68 ms | 7.94 ms | 13.8 ms | 11× slower | 32× slower | 55× slower |
| numeric loop, 1M iterations | 1.20 ms | 1.83 ms | 14.0 ms | 294 ms | 1.5× slower | 12× slower | 245× slower |
| array fill and sum, 100k | 0.45 ms | 1.63 ms | 4.03 ms | 8.45 ms | 3.6× slower | 8.9× slower | 19× slower |
| records, 10k tables | 0.13 ms | 1.09 ms | 1.29 ms | 4.23 ms | 8.7× slower | 10× slower | 34× slower |
| closures, 100k | 0.35 ms | 9.55 ms | 7.80 ms | 13.6 ms | 27× slower | 22× slower | 39× slower |
| sort 10k with comparator | 1.94 ms | 9.52 ms | 7.41 ms | 14.0 ms | 4.9× slower | 3.8× slower | 7.2× slower |
| string build, 10k pieces | 0.57 ms | 0.99 ms | 1.04 ms | 93.4 ms | 1.7× slower | 1.8× slower | 165× slower |
| calls into Go, 100k | 0.35 ms | 3.21 ms | 2.76 ms | 7.66 ms | 9.2× slower | 7.9× slower | 22× slower |
| plasma frame | 0.30 ms | 1.66 ms | 2.19 ms | 5.70 ms | 5.5× slower | 7.2× slower | 19× slower |
| particles frame | 0.006 ms | 0.22 ms | 0.36 ms | 1.59 ms | 38× slower | 61× slower | 270× slower |
<!-- /suite-table -->
